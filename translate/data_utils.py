...
def create_vocabulary_cn(vocabulary_path, data_path, max_vocabulary_size,
                      tokenizer=None, normalize_digits=True):
  """Create vocabulary file (if it does not exist yet) from data file.
    ...
    with gfile.GFile(data_path, mode="rb") as f:
      ...
      for line in f:
        ...
        if counter % 100000 == 0:
          print("  processing line %d" % counter)
        line = tf.compat.as_bytes(line)
        tokens = tokenizer(line) if tokenizer else basic_tokenizer_cn(line)
        for w in tokens:
          word = _DIGIT_RE.sub(b"0", w) if normalize_digits else w
          if word in vocab:
            vocab[word] += 1
...
def sentence_to_token_ids_cn(sentence, vocabulary,
                          tokenizer=None, normalize_digits=True):
  """Convert a string to list of integers representing token-ids.
  ...
  if tokenizer:
    ...
    words = basic_tokenizer_cn(sentence)
  if not normalize_digits:
    return [vocabulary.get(w, UNK_ID) for w in words]
  # Normalize digits by 0 before looking words up in the vocabulary.
  return [vocabulary.get(_DIGIT_RE.sub(b"0", w), UNK_ID) for w in words]
...
def data_to_token_ids_cn(data_path, target_path, vocabulary_path,
                      tokenizer=None, normalize_digits=True):
  """Tokenize data file and turn into token-ids using given vocabulary file.
    ...
    with gfile.GFile(data_path, mode="rb") as data_file:
      with gfile.GFile(target_path, mode="w") as tokens_file:
        ...
        for line in data_file:
          ...
          if counter % 100000 == 0:
            print("  tokenizing line %d" % counter)
          token_ids = sentence_to_token_ids_cn(line, vocab, tokenizer,
                                            normalize_digits)
          tokens_file.write(" ".join([str(tok) for tok in token_ids]) + "\n")
...
def prepare_myown_data(data_dir, en_vocabulary_size, cn_vocabulary_size, tokenizer=None):
...
def prepare_wmt_data1(data_dir, en_vocabulary_size, fr_vocabulary_size, tokenizer=None):
  """Get WMT data into data_dir, create vocabularies and tokenize data.
...
def prepare_own_data(data_dir, en_vocabulary_size, fr_vocabulary_size, tokenizer=None):
  """Get WMT data into data_dir, create vocabularies and tokenize data.
  ...
  #train_path = get_wmt_enfr_train_set(data_dir)
  #dev_path = get_wmt_enfr_dev_set(data_dir)
  train_path = "/home/chengzhu/work/seq2seq_translation/TED"
  #data_dir = "/home/chengzhu/work/seq2seq_translation/TED."
  # Create vocabularies of the appropriate sizes.
  en_vocab_path = os.path.join(data_dir, "TED.vocab%d.en" % en_vocabulary_size)
  cn_vocab_path = os.path.join(data_dir, "TED.vocab%d.cn" % cn_vocabulary_size)
  create_vocabulary(en_vocab_path, train_path + ".en", en_vocabulary_size, tokenizer)
  create_vocabulary(cn_vocab_path, train_path + ".cn", cn_vocabulary_size, tokenizer)

  # Create token ids for the training data.
  en_train_ids_path = train_path + (".ids%d.en" % en_vocabulary_size)
  cn_train_ids_path = train_path + (".ids%d.cn" % cn_vocabulary_size)
  data_to_token_ids(train_path + ".en", en_train_ids_path, en_vocab_path, tokenizer)
  data_to_token_ids(train_path + ".cn", cn_train_ids_path, cn_vocab_path, tokenizer)
